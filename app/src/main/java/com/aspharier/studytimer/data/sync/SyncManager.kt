...
class SyncManager @Inject constructor(
    ...
    suspend fun sync(): Result<Unit> = withContext(Dispatchers.IO) {
        val uid = auth.currentUser?.uid ?: return@withContext Result.failure(Exception("Please sign in with Google first!"))
        
        try {
            val userDocRef = firestore.collection("users").document(uid)

            // Touch user doc to ensure parent document exists
            runCatching {
                userDocRef.set(mapOf("lastActive" to System.currentTimeMillis()), com.google.firebase.firestore.SetOptions.merge()).await()
            }
